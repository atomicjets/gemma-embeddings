...
# ----------------------- Config via env -----------------------
URLS = os.getenv("TEI_URLS")
if URLS:
    TEI_URLS = [u.strip() for u in URLS.split(",") if u.strip()]
else:
    TEI_URLS = [os.getenv("TEI_URL", "http://localhost:8080/embed")]

_rr_idx = 0
def pick_url():
    global _rr_idx
    url = TEI_URLS[_rr_idx % len(TEI_URLS)]
    _rr_idx += 1
    return url
...
# Search space
BATCH_GRID = [int(x) for x in os.getenv("BATCH_GRID", "32").split(",")]
CONC_GRID = [int(x) for x in os.getenv("CONC_GRID", "128,192,256,384").split(",")]
...
# Load shaping
BUCKETING = bool(int(os.getenv("BUCKETING", "1")))  # 1=on, 0=off
AUTO_TRUNC = 2048  # TEI will also truncate if started with --auto-truncate

# Output
CSV_PATH = os.getenv("CSV_PATH", "autotune_results.csv")
# --------------------------------------------------------------
...
def bucket_for(n: int) -> tuple[int, int]:
...
def gen_batches(total_docs: int, batch_size: int) -> list[list[str]]:
    """Generate batches (optionally bucketed by token length)."""
    docs = [synth_doc() for _ in range(total_docs)]
    ids = tokenizer(
        docs, add_special_tokens=False, truncation=True, max_length=AUTO_TRUNC
    )["input_ids"]

    if BUCKETING:
        bins: dict[tuple[int, int], list[str]] = defaultdict(list)
        for d, t in zip(docs, ids):
            bins[bucket_for(len(t))].append(d)
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        ...
        async def worker() -> None:
            while True:
                ...
                # token estimate
                toks = tokenizer(
                    batch, add_special_tokens=False, truncation=True, max_length=AUTO_TRUNC
                )["input_ids"]
                ...
                try:
                    async with session.post(pick_url(), json=payload, timeout=60) as r:
                        if r.status != 200:
                            err_ctr.append(1)
